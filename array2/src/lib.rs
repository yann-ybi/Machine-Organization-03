...
impl<T: Clone> Array2<T>{
    ...
    pub fn iter_row_major(&mut self) -> impl Iterator <Item = (usize, usize)> {

        let _len = self.width_height.0 * self.width_height.1;
        let mut xyi_path: Vec<(usize, usize)> = Vec::new();

        for _i in 0.._len{
            xyi_path.push((self.col_row.0, self.col_row.1));
            
            self.next();
        }
        return xyi_path.into_iter();
    }
    pub fn iter_col_major(&mut self)-> impl Iterator <Item = (usize, usize)>{ 
